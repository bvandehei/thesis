...
public class QualityModel {
  ...
  //Adds a release to the hashtable
   ...
   public static void getReleases(String projName) throws IOException, JSONException  {
         ...
         /* This was just a check that all releases were ordered correctly
         for ( i = 0; i < releases.size();i++) {
            if ( (i + 1) < releases.size()) {
               if (releases.get(i).isAfter(releases.get(i + 1)))
                  System.out.println("ERROR: Date of previous release: " + releases.get(i).toString()
                                      + " Date of next release: " + releases.get(i+1).toString());
            }
         } */
         return;
   }
   ...
   public static void main(String[] args) throws IOException, JSONException {
      ...
      try {
         ...
         fileWriter = new FileWriter("QualityModelTemp.csv");
         //Header for CSV
